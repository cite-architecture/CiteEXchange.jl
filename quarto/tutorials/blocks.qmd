...
CEX data source organize content in blocks. Use the `blocks` function to read source data into a Vector of `Block` objects. The first parameter is a string identifying the source. The second is one of `UrlReader`, `FileReader` or `StringReader` to specify how the source should be read. We'll start by reading a CEX source on the internet.
